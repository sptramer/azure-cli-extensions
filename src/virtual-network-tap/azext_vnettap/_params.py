...
def load_arguments(self, _):
    ...
    with self.argument_context('network nic vtap-config') as c:
        c.argument('network_interface_name', options_list='--nic-name', help='Name of the network interface (NIC).')
        c.argument('vnet_tap', help='Name or ID of the virtual network tap.', validator=validate_vnet_tap)
        c.argument('vtap_config_name', options_list=['--name', '-n'], help='Name of the virtual network tap configuration.')
